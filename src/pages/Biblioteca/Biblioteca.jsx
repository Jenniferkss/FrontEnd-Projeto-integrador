...
function nomeAutor(autor, language, selectField) {
  if (!autor)
    return language === 'en' ? 'Author not informed' : 'Autor não informado'
  if (Array.isArray(autor)) {
    const a = autor[0]
    if (!a)
      return language === 'en' ? 'Author not informed' : 'Autor não informado'
    if (typeof a === 'object')
      return selectField(a, 'nome') || a.nome || String(a)
    return String(a)
  }
  if (typeof autor === 'object') {
    return (
      selectField(autor, 'nome') ||
      autor.nome ||
      (language === 'en' ? 'Author not informed' : 'Autor não informado')
    )
  }
  return String(autor)
}
    ...
    async function carregar() {
      try {
        ...
        const lista = (Array.isArray(data) ? data : [])
          .filter(
            item => item.statusApi === 'Online' && Array.isArray(item.conteudo)
          )
          .map(item => {
            ...
            const livro = {
              id: item.id,
              titulo:
                selectField(c, 'titulo') ||
                c.titulo ||
                item.livro ||
                (language === 'en' ? 'Untitled' : 'Sem título'),
  ...
  if (erro) {
    return (
      <div className={styles.page}>
        <Header />
        <div className={styles.loading}>
          {language === 'en' ? 'Could not load library.' : erro}
        </div>
        <Footer />
      </div>
    )
      ...
      <main className={styles.main}>
        <section className={styles.banner}>
          ...
          <h1 className={styles.titulo}>
            {language === 'en' ? 'Integrated Library' : 'Biblioteca Integrada'}
          </h1>
          <p className={styles.subtitulo}>
            {language === 'en'
              ? 'Works from multiple sources in one place.'
